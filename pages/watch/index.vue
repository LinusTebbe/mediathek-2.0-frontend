<template>
    <div class="container">
        <video id="video" :poster="episode.thumbnail_path"
               @timeupdate="progress = Math.floor($event.target.currentTime)" controls>
...
<script>
    ...
    export default {
        ...
        watch: {
            async progress() {
                Episode.update({where: this.videoId, data: {progress: this.progress}});
